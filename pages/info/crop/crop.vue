...
<script>
	...
	export default {
		...
		data() {
			return {
				name:'杨大宝',
				imageSrc: 'http://mrjx.weasing.cn/defaultimg.png',
				isShowImg: false,
				// 初始化的宽高
		...
		onShow () {
			if (uni.getStorageSync("headimgurl") != null) {
				this.imageSrc = uni.getStorageSync("headimgurl")
			}
		},
		methods: {
			...
			getImageInfo() {
				...
				ctx.draw(true, () => {
					...
					let pixelRatio = _this.getPixelRatio()
			...
			uploadPic (filePaths) {
				uni.uploadFile({
					...
					success: (res) => {
						...
						uni.request({
							url: "http://mrjx.weasing.cn/index.php/v1/mycenter/index",
							method: 'POST',
							data: {
